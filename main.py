...
class SpotifyReleasesTracker:
    def __init__(self) -> None:
        ...
        self.token = SpotifyTokenRefresher().refresh()
    ...
    def get_artist_code(self, artist) -> str:
        ...
        if bad_search:
            raise Exception(
                "Are you sure the artist name is right? We couldn't find it.")
        if not bad_search:
            return artist_id
    ...
    def get_most_recent_album(self, artist_id) -> Union[datetime, str, str]:
        ...
        '''
        If you want to set a specific type(album, single, compliation) and
        market use like the following:
        {
            "limit": 50,
            ...
            for i in loc["items"]:
                if "release_date" in i:
                    if len(i["release_date"]) == 10:
                        release_date = datetime.strptime(i["release_date"],
                                                         "%Y-%m-%d")
    ...
    def show_current_releases(self) -> None:
        ...
        print(f"-Release Date: \
            {utils.epoch_to_str(target_artist['release_date'])}")
